...
# File: pose.py
# Date: 7th Jan 2022
# Description: 2D pose representation used by the navigation controller
# Author: Terry Payne (trp@liv.ac.uk)


# Provides position, orientation, and geometric helper operations for
# autonomous exploration and occupancy-grid mapping.
...
class Pose:
    ...
    def range_angle(self, t):
        r""" Ensure that angle t is in the range -\pi..\pi """
        if (t > math.pi):
            t = -(2 * math.pi) + t
        elif (t < -math.pi):
            t = (2 * math.pi) + t
